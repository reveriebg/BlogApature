class PostController{
    ...
    createPost(requestData){
        if(requestData.title.length < 10){
            showPopup('error', "Post title must have atleast 10 symbols!");
            return;
        }

        if(requestData.content.length < 50){
            showPopup('error', "Post content must have atleast 50 symbols");
            return;
        }
        ...
        this._requester.post(requestUrl, requestData,
            ...
            function error(data) {
                showPopup('error',"An error na occurred while attempting" +
